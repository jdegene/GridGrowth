...
plt.imshow(t_ras.t_names_ar, interpolation=None)
plt.show()

plt.imshow(t_ras.t_ar, interpolation=None)
plt.show()

dimensions = (100,100)

terrain = np.ones(dimensions, dtype="int") + 9
terrain[15:75,83:85] = 0

# create random cost array. Higher means cell is populated faster
cost_array = np.random.randint(15, size=dimensions)
cost_method = "add" # add or multipy 

# create random terrain array. Used to determine if certain transitions between types are allowed
terrain_ar = np.random.randint(4, size=dimensions)

# set terrain rules. Values must be the same as in terrain_ar. 
# Rules are read from:to -> 'only_allowed':{2:7} means that from type 2 only a transition to
#    type 7 is allowed an no other
# Non specified transitions are considered allowed
# 'limit_into' accepts a list of transition_to codes. All connections flowing INTO these codes will be 
#   set to 0 BUT can be overwritten by "only_allowed". Allows for bulk restriction of INTO rules.
# 'takes_precedence' rules overwrite strength rules, if other cell would be stronger and fill central value
# 'takes_precedence' and 'only_allowed' can contain overlapping items
# ''only_allowed', 'never_allowed' and 'takes_precedence' can have all have lists as items
terrain_rules_dict = {'only_allowed': {2: 3, 1:0},
                     'limit_into': [0],
                     'never_allowed': {3: [2, 1]},
                     'takes_precedence': {0: 1}}
full_transition_rules_dict = build_transition_rules_full(trans_rules_dict=terrain_rules_dict, 
                                                         terrain_ar=terrain_ar)

# create test array with only 1s and some seeding starting points
tarr = np.zeros(dimensions, dtype="int") + 9999
tarr[0,9] = 2
tarr[57,95] = 3
tarr[99,99] = 2
...
total_max_value = get_possible_max_value(tarr, cost_array, weight_ar, 
                                         cost_method = cost_method, weight_method = weight_method)
...
while True:
    ...
    array_it = np.nditer(tarr, flags=['multi_index'])
    for cell in array_it:
        center_coords = array_it.multi_index
        
        # skip cells already assigned to value
        if tarr[center_coords] != nan_value:
            continue
        
        #three_by_three_arr = tarr[center_coords[0]-1:center_coords[0]+2, 
        #                          center_coords[1]-1:center_coords[1]+2]
        
        three_by_three_arr = get_3x3_array(tarr, center_coords, nan_value)
        
        if len(np.unique(three_by_three_arr)) == 1:
            continue
        
        three_by_three_marr = np.ma.masked_equal(three_by_three_arr, nan_value)
        
        # add the weight of the center value of the cost array
        if cost_array is not None:
            if cost_method == "add":
                three_by_three_marr = three_by_three_marr + cost_array[center_coords]
            elif cost_method == "mulitply":
                three_by_three_marr = three_by_three_marr * cost_array[center_coords]
        
        # add weight array values if any were given
        if weight_ar is not None:
            if weight_method == "add":
                three_by_three_marr = three_by_three_marr + weight_ar
            elif weight_method == "multiply":
                three_by_three_marr = three_by_three_marr * weight_ar
        ...
        # get a list of all cells that could set the center value
        pos_coords_list = determine_center_values(inv_three_by_three_ar, step, nan_value)
        if pos_coords_list is not None:
            
            # check which of possbile many eligible cells is allowed by terrain rules to move to center coord
            inherit_from_coord = transition_rules_outcome(full_rules_dict=full_transition_rules_dict, 
                                     strength_3x3_ar=inv_three_by_three_ar,
                                     terrain_ar=terrain_ar, 
                                     pos_coords_list=pos_coords_list, 
                                     glob_center_coord=center_coords)
            ...
            # set value 
            tarr[center_coords] = tarr[inherit_from_coord_glob]
            
        else:
            continue
    
    if np.array_equal(old_arr, tarr) and step == total_max_value:
        break
    
    # increase step, reduce step to 1 when max possible value has been reached
    step = step+1 if step <= total_max_value else 1
    iterations += 1
